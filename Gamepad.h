...
/**
 * Handle input from standard Gamepads such as the Logitech Dual Action or
 * RumblePads.
 *
 ...
 * @author Adapted from WPILib Joystick by Dave Doerr, Rodney Gleason
 */
class Gamepad
{
public:
	...
	explicit Gamepad(unsigned port);
	Gamepad(unsigned port, unsigned numAxisTypes, unsigned numButtonTypes);
	virtual ~Gamepad();
	...
	static Gamepad* GetStickForPort(unsigned port);

 ...
 */
void DashboardDataSender::sendIOPortData(double TargetValue, int AutoNumber, int AutoDelay) {
	int Mode;
	if (IOTimer->Get() < 0.1)
		return;
	IOTimer->Reset();
...
//					dash.AddFloat((float) AnalogModule::GetInstance(1)->GetAverageVoltage(i));
		...
		dash.AddDouble((double) TargetValue);
		dash.AddU8((int) AutoNumber);
		dash.AddU8((int) AutoDelay);
		dash.AddU8((int) Mode);
	}
	dash.FinalizeCluster();
	dash.Finalize();
